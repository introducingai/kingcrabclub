...
export async function GET(request: NextRequest) {
  ...
  if (!itemType) {
    return NextResponse.json(
      {
        ok: false,
        error: "Missing required itemType parameter.",
        status: 400,
  ...
  const upstream = new URL(`${baseUrl()}/api/marketplace/stats`);
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Kintara stats proxy failed", {
    ...
    console.error("Kintara stats proxy crashed", {
      source: upstream.toString(),
