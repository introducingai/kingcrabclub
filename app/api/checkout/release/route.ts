...
import type { ReleaseInput } from "@/lib/kintaraOfficial/types";

export async function POST(request: NextRequest) {
  const action = "checkout.release";
  ...
  const input = (await request.json().catch(() => ({}))) as Partial<ReleaseInput>;
  ...
  try {
    const reservation = getKintaraOfficialClient().release(input as ReleaseInput);
